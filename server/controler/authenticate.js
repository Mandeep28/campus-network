...
const loginStudent = async(req, res , next) => {
    ...
    if (!findStudent) {
        return  next(customErrorHandler("Please provide the correct credentials", 401));
     }
     const passwordChecked = await bcrypt.compare(password, user.password);

     if (!passwordChecked) {
       return next(customErrorHandler("Please Login With correct Creditials", 401));
     }
...
const getStudentDetail = async(req, res, next)=>{
    ...
    if(!student) {
        return next(customErrorHandler(`Student not found with id : ${userId}`, 404));
    }
